...
const ChatContainer: React.FC<ChatContainerProps> = ({ isSidebarOpen }) => {
  ...
  return (
    <div
      className={`flex flex-col items-center justify-between h-full text-white transition-all duration-300 ${isSidebarOpen ? "ml-64" : "ml-0"}`}
      style={{
        width: "100%",
        overflowX: "hidden",
      }}
    >
      {/* Chat messages */}
      <div
        className="flex-1 mb-2 w-full max-w-3xl flex flex-col gap-4"
        style={{
        ...
        {messages.length > 0 ? (
          messages.map((msg, index) => (
            ...
            >
              ...
              {/* Message Text */}
              <div
                className={`px-4 py-2 rounded-lg relative flex items-center ${msg.sender === "user" ? "bg-[linear-gradient(135deg,_#61ACEF,_#9987ED,_#B679E1,_#9791DB,_#74BDCC,_#59D2BF)] text-black" : "border border-gray-700 text-white"}`}
                style={{
                  ...
                  paddingBottom: "24px", // Space for the icons
                  marginTop:"15px"
                }}
              >
                {msg.text}
                ...
                <div className="absolute bottom-[-20px] right-0 flex items-center space-x-2">
                  <FontAwesomeIcon
                    icon={faVolumeHigh}
                    className="text-[#ffffff] cursor-pointer hover:text-gray-600"
                  />
                  <FontAwesomeIcon
                    icon={faCopy}
                    className="text-[#ffffff] cursor-pointer hover:text-gray-600"
                ...
                {msg.sender === "user" && (
                  <FontAwesomeIcon
                    icon={faPen}
                    className="absolute left-[-30px] top-1/2 transform -translate-y-1/2 text-[#ffffff] cursor-pointer hover:text-gray-600"
                  />
                )}
              </div>
